...
def get_trade_id(id:int, trades_service: TradesServiceDep):

    trade = trades_service.read_id(id)
    if trade:
        return trade
...
def get_trades_backtest_run_number(backtest_run_number:int, trades_service: TradesServiceDep):

    trades= trades_service.read_run_number(backtest_run_number)
    if trades: 
        return trades
...
def delete_trade(id:int, trades_service: TradesServiceDep):

    trade = trades_service.delete_id(id)
    if trade:
...
def delete_trades(backtest_run_number:int, trades_service: TradesServiceDep):

    trades = trades_service.delete_run_number(backtest_run_number)
    if trades:
        return {'message': f"Trades with backtest run number {backtest_run_number} have been deleted"}
    raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Backtest run number {backtest_run_number} doesn't exist")