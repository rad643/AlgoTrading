...
async def create_backtest(config: BacktestConfig, session: SessionDep)->dict[str, Any]: 
    ...
    ticker_df = hist_data(state.symbol, timeframe='1Day', start="2024-01-16", end="2026-01-13", limit=1000) 
    engine=TradingEngine.backtest_run(state, ticker_df[state.symbol]) # run the entire backtest once -> this is going to be your engine 
