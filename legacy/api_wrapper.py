...
from legacy.sqlite_log_events_db import LogEventsDatabase
from legacy.sqlite_summary_db import SummaryDatabase
from legacy.sqlite_trades_db import TradesDatabase
...
summary_db = SummaryDatabase(
    "summary"
)  # `summary.db` is the database and `summary` is the table inside it
log_events_db = LogEventsDatabase(
    "log_events"
)  # `log_events.db` is the database and `log_events` is the table inside it
...
def create_backtest(config: BacktestConfig) -> dict[str, Any]:
    ...
    engine = TradingEngine.backtest_run(state)  # type: ignore [call-arg]

    # compute final run data frame
    run_df = TradingEngine.performance_metrics_data_frame(state)
    run_df = run_df.where(
        cond=run_df.notna(), other=None
    )  # replaces all Nan values from the data frame with None
    run_df = run_df.to_dict(orient="records")  # type: ignore [assignment]
    run_df = run_df[0]  # type: ignore [assignment]
    summary_with_id = summary_db.create_summary(
        run_df
    )  # creating the database table after computing the run data frame from the engine

    # compute the log events
    log_events = engine["log_events"]
    log_events = log_events.where(cond=log_events.notna(), other=None)
    log_events = log_events.to_dict(orient="records")  # type: ignore [assignment]
    log_events_with_id = log_events_db.create_event(log_events)

    # compute the trades
    trades = engine["trades"]
    trades = trades.where(cond=trades.notna(), other=None)
    trades = trades.to_dict(orient="records")  # type: ignore [assignment]
...
def delete_log_event(backtest_run_number: int):
    ...
    deleted_rows = log_events_db.delete_log_event_run_number(
        backtest_run_number, "log_events"
    )
