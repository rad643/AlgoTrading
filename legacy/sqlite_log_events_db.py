...
class LogEventsDatabase:
    ...
    def create_event(self, df:pd.DataFrame)->dict[str,Any]:
    ...
    def read_event(self,backtest_run_number,name)->dict[str,Any] | None:
    ...
    def delete_log_event_run_number(self,backtest_run_number,name):
        ...
        self.cursor.execute(f"select * from {name} where run_number=?", (backtest_run_number, ))
        rows=self.cursor.fetchall()

        self.cursor.execute(f"delete from {name} where run_number=?", (backtest_run_number, ))
        self.connection.commit()
