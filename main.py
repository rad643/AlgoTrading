...
class TradingEngine:
    ...
    @staticmethod
    def build_one_completed_trade_row(
        run_number: int,
        ticker: str,
        strategy: str,
        entry_day: int,
    ...
    def position(state: ExecutionState) -> int:
        ...
        return (
            ...
            else state.positionMeanReversion
        )
    ...
    def entry_price(state: ExecutionState) -> float:
        ...
        return (
            ...
            if TradingEngine.strategy(state) == "Trend"
            else state.entryPriceMeanReversion
        )
    ...
    def exit_price(state: ExecutionState) -> float:
        ...
        return (
            ...
            if TradingEngine.strategy(state) == "Trend"
            else state.exitPriceMeanReversion
        )
    ...
    def profit(state: ExecutionState) -> float:
        ...
        return (
            state.profitTrend
            if TradingEngine.strategy(state) == "Trend"
            else state.profitMeanReversion
        )
    ...
    def backtest_start_logging_event(state: ExecutionState) -> None:
        ...
        event_log_row = TradingEngine.build_event_log_row(
    ...
    @staticmethod
    def backtest_end_logging_event(
        state: ExecutionState, day: int, my_date: date
    ) -> None:
    ...
    @staticmethod
    def generator(
        state: ExecutionState, one_df: pd.DataFrame
    ...
    def build_dictionary_prices(
        ...
        price_row = TradingEngine.build_price_row(
            day,
            my_date,
            state.ticker_name,
            TradingEngine.strategy(state),
    ...
    @staticmethod
    def process_one_day(
        state: ExecutionState, day, my_date, closingPrice, average, nextDayOpeningPrice
    ) -> tuple[Any, ...]:
        """
    ...
    def build_dictionary_trades(state: ExecutionState) -> None:
        ...
        one_completed_trade_row = TradingEngine.build_one_completed_trade_row(
            ExecutionState.backtest_run_number + 1,
            state.ticker_name,
            TradingEngine.strategy(state),
            state.entry_day,
            ...
            round(
                (
                    (TradingEngine.exit_price(state) - TradingEngine.entry_price(state))
                    / TradingEngine.entry_price(state)
                )
                * 100,
                2,
    ...
    @staticmethod
    def run_days_one_and_two(
        state: ExecutionState, day: int, my_date: date, closingPrice: float
    ) -> None:

        state.listStoreEquityValues.append(state.equity)

        TradingEngine.build_dictionary_prices(state, day, my_date, closingPrice, None)
    ...
    def build_log_events_data_frame(state: ExecutionState) -> pd.DataFrame:
        ...
        loggingEventsDataFrame["day"] = loggingEventsDataFrame["day"].astype(
            pd.Int64Dtype()
        )
    ...
    def build_trades_data_frame(state: ExecutionState) -> pd.DataFrame:
        ...
        if tradesDataFrame.empty:
            ...
            tradesDataFrame["number_trades_took_place"] = (
                tradesDataFrame.groupby(by="run_number").cumcount() + 1
            )
    ...
    def build_drawdown_series_data_frame(state: ExecutionState) -> pd.DataFrame:
        ...
        for i in range(len(state.listStoreEquityValues)):
            row_drawdown_series = TradingEngine.build_drawdown_series(
                i + 1,
            ...
            list_dictionaries_rows_per_equity_drawdown_series.append(
                row_drawdown_series
            )
        # drawdown series final computed data frame from all dictionary rows
        drawdown_series = pd.DataFrame(
            data=list_dictionaries_rows_per_equity_drawdown_series
        )
        # add another extra column in the drawdown series data frame called 'labels' that uniquely identifies each line
        drawdown_series["labels"] = (
            drawdown_series["ticker"] + "-" + drawdown_series["strategy"]
        )
    ...
    def build_equity_curve_data_frame(state: ExecutionState) -> pd.DataFrame:
        ...
        equity_curve_data_frame = drawdown_series[
            ["day", "run_number", "ticker", "strategy", "equity", "labels"]
        ]
    ...
    def backtest_run(
        ...
        for extracted_tuple in dl.read_ticker_dataframe(
            ...
            # Days 1 and 2
                ...
                TradingEngine.run_days_one_and_two(state, day, my_date, closingPrice)
        ...
        dictionary_data_frames = TradingEngine.build_data_frames(
            log_events_data_frame,
    ...
    def performance_metrics_data_frame(state: ExecutionState) -> pd.DataFrame:
        ...
        mddMetric = performance_metrics.mdd(state.listStoreEquityValues)

        expectancy = TradingEngine.try_except_performance_metric(
            lambda: performance_metrics.expectancy(
        ...
        payoffRatio = TradingEngine.try_except_performance_metric(
            lambda: performance_metrics.payoff_ratio(
                stats_dictionary["positive_total"],
                stats_dictionary["negative_total"],
                stats_dictionary["positive_count"],
                stats_dictionary["negative_count"],
        ...
        profitFactor = TradingEngine.try_except_performance_metric(
            lambda: performance_metrics.profit_factor(
                stats_dictionary["positive_total"], stats_dictionary["negative_total"]
            )
        )

        sharpeRatio = TradingEngine.try_except_performance_metric(
            lambda: performance_metrics.sharpe_ratio(state.listStoreEquityValues)
        )
...
class PlottingLayer:
    ...
    def user_interface_oriented_plotting_equity_curve(self):
        ...
        equity_curve_line_chart = px.line(
            data_frame=self.results_data_frames["Equity Curve"],
            x="day",
            y="equity",
            color="labels",
    ...
    def user_interface_oriented_plotting_drawdown_series(self):
        ...
        drawdown_series_chart = px.line(
            data_frame=self.results_data_frames["Drawdown Series"],
            x="day",
            y="drawdown_pct",
            color="labels",
    ...
    def user_interface_oriented_plotting_completed_trades(self):
        ...
        drawdown_series_chart = px.bar(
            data_frame=self.results_data_frames["Completed Trades"],
            x="number_trades_took_place",
            y="profit",
            color="labels",
            title="Completed Trades Chart",
            labels={
                "number_trades_took_place": "Trade number",
                "profit": "Profit ($)",
                "labels": "run_label",
            },
            barmode="group",
    ...
    def user_interface_oriented_plotting_log_events(self):
        ...
        log_events_chart = px.scatter(
            data_frame=self.results_data_frames["Log Events"],
            x="day",
            y="event_type",
            color="labels",
    ...
    def user_interface_oriented_plotting_price_chart(self, ticker, strategy):
        ...
        ticker_strategy_data_frame_log_events = self.results_data_frames["Log Events"][
            self.results_data_frames["Log Events"]["labels"] == f"{ticker}-{strategy}"
        ]
        ticker_strategy_data_frame_buy_executed = ticker_strategy_data_frame_log_events[
            ticker_strategy_data_frame_log_events["event_type"] == "BUY_EXECUTED"
        ]
        ticker_strategy_data_frame_buy_executed_price = (
            ticker_strategy_data_frame_buy_executed["execution_price"]
        )
        ticker_strategy_data_frame_sell_executed = (
            ticker_strategy_data_frame_log_events[
                ticker_strategy_data_frame_log_events["event_type"] == "SELL_EXECUTED"
            ]
        )
        ticker_strategy_data_frame_sell_executed_price = (
            ticker_strategy_data_frame_sell_executed["execution_price"]
        )
        strategy_chart = go.Figure()

        # closing price + average (lines) -> prices data frame
        ticker_strategy_data_frame = self.results_data_frames["Prices"][
            (self.results_data_frames["Prices"]["ticker"] == f"{ticker}")
            & (self.results_data_frames["Prices"]["strategy"] == f"{strategy}")
        ]
        ...
        strategy_chart.add_trace(
            go.Scatter(
                x=ticker_strategy_data_frame["day"],
                y=ticker_strategy_closing_prices,
                mode="lines",
                name="Closing Prices",
            )
        )
        ...
        strategy_chart.add_trace(
            go.Scatter(
                x=ticker_strategy_data_frame_buy_executed["day"],
                y=ticker_strategy_data_frame_buy_executed_price,
                mode="markers",
                name="BUY_EXECUTED",
                marker_size=15,
            )
        ...
        strategy_chart.add_trace(
            go.Scatter(
                x=ticker_strategy_data_frame_sell_executed["day"],
                y=ticker_strategy_data_frame_sell_executed_price,
                mode="markers",
                name="SELL_EXECUTED",
                marker_size=15,
            )
        ...
        strategy_chart.update_layout(
            ...
            yaxis_title="Price",
            title=f"Price chart BUY/SELL markers: {ticker}-{strategy}",
        )
        strategy_chart.show()
...
class AggregationLayer:
    def __init__(self, results_data_frames: dict[str, pd.DataFrame]) -> None:
        ...
        if not isinstance(results_data_frames, dict):
            raise TypeError(
                "The resulting data frames need to be stored inside a dictionary"
            )

        self.results_data_frames = results_data_frames

    @staticmethod
    def build_average_performance_summary(
        average_total_net_profit: float,
        average_mdd: float,
    ...
    @staticmethod
    def build_aggregation_outputs(
        total_runs: int,
        best_run_summary: dict,
        worst_run_summary: dict,
        average_performance_summary: dict,
        selected_run_summary: dict | pd.Series[Any],
        selected_run_trade_list: pd.DataFrame,
    ) -> dict[str, Any]:
    ...
    def run_summary(self, use_max: bool) -> dict[str, Any]:
        ...
        if use_max:
            total_net_profit = (
                self.results_data_frames["Final Data Frame Run"]["total_net_profit"]
            ).max()
        ...
        boolean_mask_total_net_profit = (
            (self.results_data_frames["Final Data Frame Run"]["total_net_profit"])
            == total_net_profit
        )

        boolean_indexing = self.results_data_frames["Final Data Frame Run"][
            boolean_mask_total_net_profit
        ]

        first_row = boolean_indexing.iloc[
            0
        ]  # select only the first row matching that value
    ...
    def average_performance_summary(self) -> dict[str, float]:
        ...
        average_total_net_profit = round(
            self.results_data_frames["Final Data Frame Run"]["total_net_profit"].mean(),
            2,
        )

        average_mdd = round(
            self.results_data_frames["Final Data Frame Run"]["mdd"].mean(), 2
        )

        average_expectancy = round(
            self.results_data_frames["Final Data Frame Run"]["expectancy"].mean(), 2
        )

        average_payoff_ratio = round(
            self.results_data_frames["Final Data Frame Run"]["payoff_ratio"].mean(), 2
        )

        average_profit_factor = round(
            self.results_data_frames["Final Data Frame Run"]["profit_factor"].mean(), 2
        )

        average_sharpe_ratio = round(
            self.results_data_frames["Final Data Frame Run"]["sharpe_ratio"].mean(), 2
        )

        average_performance_summary = (
            AggregationLayer.build_average_performance_summary(
                float(average_total_net_profit),
                float(average_mdd),
    ...
    def selected_run_summary(self, ticker: str, strategy: str) -> pd.Series[Any]:
        ...
        selected_run_summary = boolean_index.iloc[
            0
        ]  # if multiple identical ticker/strategy pairs, pick the first pair only

        selected_run_summary = selected_run_summary.to_dict()  # type: ignore [assignment]
    ...
    def aggregation_outputs(self, ticker: str, strategy: str) -> dict[str, Any]:
        ...
        aggregation_outputs = AggregationLayer.build_aggregation_outputs(
            self.total_runs_summary(),
    ...
    def structured_data_outputs(selected_tickers: str) -> dict[str, pd.DataFrame]:
        ...
        for symbol in bars_by_symbol:
            ...
            for trendMethod in (True, False):
                state = ExperimentRunner.state(symbol, trendMethod)  # type: ignore [arg-type]
                ...
                data_frames_run.append(
                    TradingEngine.performance_metrics_data_frame(state)
                )
