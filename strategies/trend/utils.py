...
def buy(
    ...
    cashValue = round(
        cashValue
        - (positionTrend * entryPriceTrend)
        - (positionTrend * flat_fee_per_share),
        3,
    )
    # unrealized profit
    ...
    if verbose_run:
        print(
            f"Day {day} | Date: {date} | Close: {closingPrice} | Execution price: {entryPriceTrend} | Avg: {format(average, '.3f')} | Trend: {pending_action} | Position: {positionTrend} | Cash: {cashValue} | Equity: {equity}"
        )
        print("\n")
    return (
        entryPriceTrend,
...
def sell(
    ...
    if verbose_run:
        print(
            f"Day {day} | Date: {date} | Close: {closingPrice} | Execution price: {exitPriceTrend} | Avg: {format(average, '.3f')} | Trend: {pending_action} | Position: {positionTrend} | Cash: {cashValue} | Equity: {equity} | P&L: {format(profitTrend, '.3f')}"
        )
        print("\n")
    return (
        exitPriceTrend,
...
def hold(cashValue, positionTrend, closingPrice, verbose_run, day, date, average):
    """
    ...
    if verbose_run:
        print(
            f"Day {day} | Date: {date} | Close: {closingPrice} | Avg: {format(average, '.3f')} | Trend: {pending_action} | Position: {positionTrend} | Cash: {cashValue} | Equity: {equity}"
        )
        print("\n")
    return (positionTrend, cashValue, equity, pending_action)
