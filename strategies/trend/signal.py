...
def trend_step(verbose_run: bool, day:int, date:str, closingPrice:float, average:float, nextDayOpeningPrice: float, cashValue:float, equity:float, pending_action: str, positionSizing: float, flat_fee_per_share: float, fixed_bps:float, positionTrend:int, entry_day: int, exit_day: int, entryPriceTrend:float, exitPriceTrend:float, profitTrend:float)->tuple:
    ...
    if(pending_action=="BUY"):
        ...
        if(positionTrend==0): 
            entryPriceTrend,positionTrend,cashValue,equity,entry_day,pending_action=buy(day, cashValue,nextDayOpeningPrice, fixed_bps, positionSizing,flat_fee_per_share,closingPrice, verbose_run, date,average)
        else: 
            positionTrend,cashValue,equity,pending_action=hold(cashValue,positionTrend,closingPrice,verbose_run,day,date,average)

    elif(pending_action=="SELL"):

        if(positionTrend!=0):            
            exitPriceTrend,positionTrend,cashValue,equity,profitTrend,exit_day,pending_action=sell(day,cashValue,nextDayOpeningPrice,fixed_bps, entryPriceTrend,closingPrice,flat_fee_per_share,verbose_run,date,average,positionTrend)
        else: 
            positionTrend,cashValue,equity,pending_action=hold(cashValue,positionTrend,closingPrice,verbose_run,day,date,average)
